...
export function MobileInbox({ account, messages, selectedId, search, mode, onRetry, onCompose, onSelectMessage }: MobileInboxProps) {
      ...
      <div className="mt-7">
        ...
        {mode === "normal" ? messages.slice(0, 8).map((message) => (
          ...
          >
            ...
            <span className="absolute left-20 right-[84px] top-2 truncate text-[15px] text-ink data-[unread=true]:font-bold data-[unread=false]:font-semibold" data-unread={message.unread}>
              {message.sender}
            </span>
            ...
            <span className="absolute left-20 right-12 top-8 truncate text-[13.5px] text-ink">{message.subject}</span>
            {message.hasAttachment ? <Icon name="paperclip" className="absolute right-8 top-[33px] h-3.5 w-3.5 text-muted" /> : null}
            <span className="absolute left-20 right-6 top-[55px] truncate text-[12.5px] text-[#a2a8b0]">{message.snippet}</span>
          </button>
        )) : null}
      </div>
