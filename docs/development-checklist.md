...
- [x] HTML sanitizer preserves safe email layout styles while blocking active or remote CSS.
  Evidence: a property allowlist preserves colors/backgrounds, spacing, borders, typography, alignment, table layout, and image dimensions for common email HTML; style values containing `url`, `expression`, `@`, or `!` are rejected. SVG data images, forms, inputs, and buttons remain blocked.
